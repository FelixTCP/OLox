...
and var_declaration = function
  ...
  | { ttype = IDENTIFIER; _ } :: t :: _rest ->
      Error
        [
          parse_error t ("Expected '=' in variable declaration but found " ^ t.lexeme);
        ]
  | t :: _ ->
      Error
        [
...
and class_declaration (tokens : Token.token list) =
  let rec parse_body acc (tkns : Token.token list) =
    match tkns with
  ...
  | t :: _ ->
      Error
        [ parse_error t "Expected identifier after 'class' in class declaration" ]
  | [] ->
      Error
        [
          parse_error (Token.make_eof_token ())
...
and assignment tokens : (expr * Token.token list, Error.t list) result =
  ...
  | { ttype = EQUAL; _ } :: rest' -> (
      ...
      | GET (obj, name) ->
          ...
          Ok (SET (obj, name, value), rest'')
      | VARIABLE var_token ->
          assignment rest' >>= fun (value, rest'') ->
          Ok (ASSIGN (var_token, value), rest'')
      | _ -> Error [ parse_error (List.hd rest) "Invalid assignment target" ]
    )
  | _ -> logic_or tokens
...
and call tokens : (expr * Token.token list, Error.t list) result =
  let rec parse_call (expr : expr) (tokens : Token.token list) =
    match tokens with
    | { ttype = LEFT_PAR; _ } :: rest ->
    ...
    | { ttype = DOT; _ } :: rest -> (
        ...
        | _ ->
            Error
              [
                parse_error (List.hd rest)
                  "Expected property name after '.' in property access";
              ]
      )
    | _ -> Ok (expr, tokens)
  in
...
let parse tokens : (Statement.stmt list * Resolver.resolution, Error.t list) result =
