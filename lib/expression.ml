type expr =
  ...
  | GET of expr * Lexer.Token.token
  | SET of expr * Lexer.Token.token * expr
  | THIS of Lexer.Token.token

let rec expr_to_string ?(indent = 0) expr =
  ...
  | UNARY (op, sub_expr) ->
      let sub_expr_str = expr_to_string ~indent:(indent + 1) sub_expr in
      fmt "%sUnary: %s\n%s" indent_str op.lexeme sub_expr_str
  | BINARY (l_expr, op, r_expr) ->
      let left_str = expr_to_string ~indent:(indent + 1) l_expr in
      let right_str = expr_to_string ~indent:(indent + 1) r_expr in
  ...
  | ASSIGN (name, value) ->
      let value_str = expr_to_string ~indent:(indent + 1) value in
      fmt "%sAssign: %s\n%s" indent_str name.lexeme value_str
  | CALL (callee, args) ->
      let callee_str = expr_to_string ~indent:(indent + 1) callee in
      let args_str =
        args |> List.map (expr_to_string ~indent:(indent + 2)) |> String.concat ""
      in
      fmt "%sCall:\n%s  Callee:\n  %s%s  Arguments:\n%s" indent_str indent_str
        callee_str indent_str args_str
  | GET (object_expr, name) ->
      let object_str = expr_to_string ~indent:(indent + 1) object_expr in
      fmt "%sGet: %s\n%s" indent_str name.lexeme object_str
  | SET (object_expr, name, value_expr) ->
      let object_str = expr_to_string ~indent:(indent + 1) object_expr in
      let value_str = expr_to_string ~indent:(indent + 1) value_expr in
